...
function buildMarkdown(offers, capturedDate) {
  ...
  md += `**IMPORTANT — no activation was performed on this bank.** Online offers (the vast majority) need NO activation at all — they're auto-tracked whenever Harry shops with the card, nothing to do. In-Store offers DO need activation, but Capital One caps activation at only 3 CONCURRENT slots (7-day expiry each) — with ~${inStore.length} in-store offers available and only 3 usable at once, this can't be scripted the way the other banks' full-activation was. When Harry mentions a purchase matching an In-Store offer below, tell him it's available but needs manual activation on capitaloneoffers.com first (check how many of his 3 slots are free).\n\n`;
  md += `**How to use this:** when Harry mentions a purchase or asks what to buy, check this table for the merchant first. For Online offers, tell him the cash-back rate — no action needed, it's automatic. For In-Store offers, tell him he'd need to activate it manually first (limited to 3 at a time).\n\n`;
